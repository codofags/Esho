...
//{
    ...
    public class ZoomPinch : MonoBehaviour
    {
        ...
        private void EditorZoom()
        {
            ...
            {
                var cameraRigPosition = mainCamera.localPosition;

                cameraRigPosition.y += scrollDelta.y * PerspectiveZoomSpeed * 10;

                if (cameraRigPosition.y > maximumZoomLimit)
                {
                    cameraRigPosition.y = maximumZoomLimit;
                }

                if (cameraRigPosition.y < minimumZoomLimit)
                {
                    cameraRigPosition.y = minimumZoomLimit;
                }

                mainCamera.localPosition = cameraRigPosition;

            if(MapManager.Inst!=null)                MapManager.Inst.ScaleMarkers();
            }
        }


        [SerializeField] private float maximumZoomLimit = 1900;
        [SerializeField] private float minimumZoomLimit = 500;
        ...
        private void AndroidZoom()
        {
            ...
            if (Input.touchCount >= 2)
            {
                ...
                else
                {
                    ...
                    cameraRigPosition.y += deltaMagnitudeDiff * PerspectiveZoomSpeed;
